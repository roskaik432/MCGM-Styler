...
from perceptual import compute_vgg_loss, debug_images
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--lambda_attention", type=float, default=1e-2) #0.01
    ...
    parser.add_argument(
        ...
        type=str,
        default="<s>",
        help="A token to use as a placeholder for the style",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--with_vgg_loss",
        action="store_true",
        help="Use the vgg loss",
    )
    parser.add_argument(
        "--with_vgg_loss_and_mask",
        action="store_true",
        help="Use the vgg loss and mask applied to pred",
    )
    parser.add_argument(
        "--with_image_target",
        action="store_true",
        help="Use the vgg loss and mask applied to the target as reference",
    )
    parser.add_argument(
        "--debug_image",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=float,
        default=1,
        help="Lambda for VGG loss",
    )
    parser.add_argument(
        "--lambda_prior_loss",
        type=float,
        default=1,
        help="Lambda for proir loss",
    )
    parser.add_argument(
        "--disable_mse_loss_phase1",
        action="store_true",
        default=False,
        help="If we disable mse loss on phase 1",
    )
    parser.add_argument(
        "--disable_mse_loss_phase2",
        action="store_true",
        default=False,
        help="If we disable mse loss on phase 2",
    )

    parser.add_argument(
        "--disable_VGG_loss_phase1",
        action="store_true",
        default=False,
        help="If we disable VGG loss on phase 1",
    )
    parser.add_argument(
        "--disable_VGG_loss_phase2",
        action="store_true",
        default=False,
        help="If we disable VGG loss on phase 2",
    )
    parser.add_argument(
        "--soft_masks",
        action="store_true",
        default=False,
        help="If we use soft masks",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="used to add the identifier token to the prompt",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="If we use masked diversity",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="If we use diversity loss",
    )

    parser.add_argument(
        "--lambda_diversity_loss",
        type=float,
        default=1,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="strategy for the mask encoder 1-2-3 1: use the mask taken M0,MM MM,M1 M0,M1 M0,M1 2: use the mask taken M0,MM M1,MM M0,M1 M1,M0 3: use the mask taken M0,MM MM,M1 M0,M1 M1,M0 ",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Enable mask encoder (mask conditioning)",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Enable Stago code for prior mask generation",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="calculate the cross attention on the mask and adding it to the assets loss",
    )

    parser.add_argument(
        "--cross_att_on_mas_weight",
        type=float,
        default=.3,
        help="weight the cross attention on mask",
    )
...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        num_images = len(glob.glob(os.path.join(instance_data_root, "img*.jpg")))
#-------note 
# add many input images
        self.instance_image = []
        self.instance_masks = []

        for j in range(num_images):
            instance_img_path = os.path.join(instance_data_root, f"img{j}.jpg")
            self.instance_image.append(self.image_transforms(Image.open(instance_img_path)))
            print('load image ', instance_img_path)
            instance_masks = []
            for i in range(num_of_assets):
                try:
                    instance_mask_path = os.path.join(instance_data_root, f"mask{j}{i}.png")
                    ...
                    print('load mask ', instance_mask_path)
                except FileNotFoundError:
                    break
            self.instance_masks.append(torch.cat(instance_masks))
    ...
    def __getitem__(self, index):
        example = {}
        index_img = index % self._num_images

        # FIXME temporary fix for the number of tokens
        num_of_tokens = random.randrange(1, len(self.instance_masks[index_img]) + 1)
        #num_of_tokens = len(self.instance_masks[index_img])
        ...
        prompt = "the style of " + " and ".join(tokens_to_use)
        #print('tokens_to_use: ', tokens_to_use)
        print('prompt: ', prompt)

        example["instance_images"] = self.instance_image[index_img]
        example["instance_masks"] = self.instance_masks[index_img][tokens_ids_to_use]  
                  
        # inject empty tokens for mask conditioning based on the strategy
        if self.strategy == 2:
            new_masks = torch.zeros((len(self.instance_masks[index_img]),) + example["instance_masks"].shape[1:]).float()
            new_masks[tokens_ids_to_use] = example["instance_masks"]
            example["instance_masks"] = new_masks

        if self.strategy == 3:
            new_masks = torch.zeros((len(self.instance_masks[index_img]),) + example["instance_masks"].shape[1:]).float()
            if (len(example["instance_masks"]) == 1):
                new_masks[tokens_ids_to_use] = example["instance_masks"]
            else:
                new_masks = self.instance_masks[index_img][tokens_ids_to_use]  
            example["instance_masks"] = new_masks

        if self.strategy == 4:
            new_masks = torch.zeros((len(self.instance_masks[index_img]),) + example["instance_masks"].shape[1:]).float()
            if (len(example["instance_masks"]) == 1):
                new_masks[0] = self.instance_masks[index_img][tokens_ids_to_use]
            else:
                new_masks = self.instance_masks[index_img][tokens_ids_to_use]
            example["instance_masks"] = new_masks
...
class SpatialDreambooth:
    def __init__(self):
        self.args = parse_args()
         #-------tensorBoard 
        self.writer = SummaryWriter(self.args.output_dir + '/tensorboard/train_{}'.format(datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")))
         #--------
        self.main()

    def main(self):
        #Stego = LitUnsupervisedSegmenter.load_from_checkpoint("stego-checkpoint\cocostuff27_vit_base_5.ckpt").cuda()
        #Stego = LitUnsupervisedSegmenter.load_from_checkpoint("../hpc/home/rami.skaik/mybreak-a-scene/stego-checkpoint/cocostuff27_vit_base_5.ckpt").cuda()
        

        self.disable_mse_loss = self.args.disable_mse_loss_phase1
        self.disable_VGG_loss = self.args.disable_VGG_loss_phase1
      #  token_prompt = self.args.token_prompt
...
# Stego code 
                        # if self.args.with_Stego:

                        #     with torch.no_grad():
                        #         img_prime = TF.to_tensor(image) #added after

                        #         code = Stego(img_prime)
                        #         # code2 = model(img.flip(dims=[3]))
                        #         # code  = (code1 + code2.flip(dims=[3])) / 2
                        #         # code = F.interpolate(code, img.shape[-2:], mode='bilinear', align_corners=False)
                        #         linear_probs = torch.log_softmax(Stego.linear_probe(code), dim=1).cpu()
                        #         #cluster_probs = model.cluster_probe(code, 2, log_probs=True).cpu()


                        #         single_img = img_prime[0].cpu()
                        #         linear_pred = dense_crf(single_img, linear_probs[0]).argmax(0)
                        #         linear_pred = (linear_pred == 7 ) * 255
                        #         mask = linear_pred > 0

                        #         image_filename1 = (
                        #         class_images_dir
                        #         / f"mask_{example['index'][i] + cur_class_images}-{hash_image}.png"
                        #     )
                        #         mask.save(image_filename1)
...
#---------end note for stego
        ...
        # load mask encoder
        self.mask_encoder = MaskEncoder().to(self.accelerator.device)
...
#------note
        ...
        # FIXME only one mask for each image is allowed
    ...
    #-- added token prompt
        #self.args.instance_prompt = "a photo of " + " and ".join(self.placeholder_tokens) + " " + self.token_prompt + " "
        self.args.instance_prompt = "the style of " + " and ".join(
            self.placeholder_tokens
        )
    ...
    #--- gaussian blur for attention masks
        #self.gaussian_blur = transforms.GaussianBlur(kernel_size=5, sigma=2.0)
        #self.gaussian_blur = transforms.GaussianBlur(kernel_size=7, sigma=10.0)
        ...
        for epoch in range(first_epoch, self.args.num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                if self.args.phase1_train_steps == global_step:
                    self.disable_mse_loss = self.args.disable_mse_loss_phase2
                    self.disable_VGG_loss = self.args.disable_VGG_loss_phase2
                ...
                with self.accelerator.accumulate(self.unet):
                    ...
                    encoder_hidden_states_text = self.text_encoder(batch["input_ids"])[0]

                    encoder_hidden_states = encoder_hidden_states_text
                    # inject mask conditioning
                    if self.args.with_mask_encoder:
                        encoder_hidden_states_mask = self.mask_encoder.to('cuda')(batch["instance_masks"])
                        
                        if len(encoder_hidden_states) > len(batch["instance_masks"]):
                            class_mask_embeds = torch.zeros_like(encoder_hidden_states_mask)
                            encoder_hidden_states_mask = torch.cat([class_mask_embeds, encoder_hidden_states_mask])
                        encoder_hidden_states = self.mask_encoder.merge(encoder_hidden_states_text, encoder_hidden_states_mask)

                    # #inject multiple masks in separate tokens
                    #     new_encoder_hidden_states = []
                    #     for i in range(len(batch["input_ids"])):
                    #         text_embedding = encoder_hidden_states_text[i]
                    #         mask_embedding = encoder_hidden_states_mask[i]
                    #         combined_embedding = torch.cat((text_embedding, mask_embedding), dim=0)
                    #         new_encoder_hidden_states.append(combined_embedding)

                    #     # Stack the new encoder hidden states to form the final tensor
                    ...
                    # make a copy of model_pred = [a, b]
                    model_pred_bck = model_pred
                    target_bck = target
                    ...
                    if self.args.with_prior_preservation:
                        ...
                        self.writer.add_scalar('losses/instance loss',loss,index)

                        # Compute prior loss
                        prior_loss = F.mse_loss(
                            model_pred_prior.float(),#batch["input_ids"],
                            target_prior.float(),
                            reduction="mean",

                        )
                        self.writer.add_scalar('losses/prior loss',prior_loss,index)

                        # Add the prior loss to the instance loss.
                        prior_loss_lam=self.args.lambda_prior_loss
                #----added lambda to prior loss                     
                        loss = (loss + self.args.prior_loss_weight * prior_loss)*prior_loss_lam * float(not self.disable_mse_loss)
                        ...
                        loss = F.mse_loss(
                            model_pred.float(), target.float(), reduction="mean"
                        ) * float(not self.disable_mse_loss)
                        
        #----added lambda to prior loss   
                        prior_loss_lam=self.args.lambda_prior_loss                    
                        loss = loss*prior_loss_lam
                        self.writer.add_scalar('losses/instance loss_Mask',loss,index)
                ...
                # #--- soft mask
                #             if self.args.soft_masks:
                #                 batch["instance_masks"][batch_idx] = self.gaussian_blur(batch["instance_masks"][batch_idx])
                # #---
                            ...
                            if self.args.with_prior_preservation:
                                curr_cond_batch_idx = self.args.train_batch_size + batch_idx #if using prior
                            else:
                                curr_cond_batch_idx = batch_idx

                            if self.args.strategy == 0 or \
                                    (self.args.strategy == 4 and len(batch["token_ids"][batch_idx]) == 2) or \
                                        (self.args.strategy == 3 and len(batch["token_ids"][batch_idx]) == 2):
                                 for mask_id in range(len(GT_masks)):
                                    ...
                                    # mse loss between 16x16 attentions and 16x16 masks
                                    ...
                                    if self.args.with_cross_att_on_mask:
                                        mask_attn_mask = agg_attn[..., encoder_hidden_states_text.shape[1] + mask_id]
                                        # normalization between 0 and 1
                                        mask_attn_mask = (
                                            mask_attn_mask / mask_attn_mask.max()
                                        )

                                        # mse loss between 16x16 attentions and 16x16 masks
                                        attn_loss += self.args.cross_att_on_mas_weight * F.mse_loss(
                                            GT_masks[mask_id, 0].float(),
                                            mask_attn_mask.float(),
                                            reduction="mean",
                                        )

                            elif self.args.strategy == 2 or \
                                    (self.args.strategy == 3 and len(batch["token_ids"][batch_idx]) == 1):

                                for idx, token_id in enumerate(batch['token_ids'][batch_idx]):
                                    curr_placeholder_token_id = self.placeholder_token_ids[
                                        token_id
                                    ]
                                    ...
                                    # mse loss between 16x16 attentions and 16x16 masks
                                    attn_loss += F.mse_loss(
                                        GT_masks[token_id, 0].float(),
                                        asset_attn_mask.float(),
                                        reduction="mean",
                                    )
                            elif self.args.strategy == 4 and len(batch["token_ids"][batch_idx]) == 1:
                                token_id = batch['token_ids'][batch_idx][0]
                        ...
                        loss += attn_loss
                        self.writer.add_scalar('losses/Attention loss',attn_loss,index)
                    ...
                    if self.args.debug_image:
                        debug_images(
                            ...
                            timesteps[-1],
                            target_bck,
                            batch['instance_masks'][-1][None],
                            batch['pixel_values'][-1][None],
                            self)
...
#                         # Add noise to the latents according to the noise magnitude at each timestep
#                         # (this is the forward diffusion process)
#                         noisy_latents1 = self.noise_scheduler.add_noise(
#                             latents.clone(), noise1, timesteps
#                         )
#                         noisy_latents2 = self.noise_scheduler.add_noise(
#                             latents.clone(), noise2, timesteps
#                         )
...
#                         # Predict the noise residual
#                         #model_pred1 = model_pred
#                         model_pred1 = self.unet(
#                             noisy_latents1, timesteps, encoder_hidden_states
#                         ).sample
#                         #model_pred2 = model_pred
#                         model_pred2 = self.unet(
#                             noisy_latents2, timesteps, encoder_hidden_states
#                         ).sample
                        
#                         if self.args.with_prior_preservation:
#                             # Chunk the noise and model_pred into two parts and compute the loss on each part separately.
#                             _, model_pred1 = torch.chunk(model_pred1, 2, dim=0)
#                             _, model_pred2 = torch.chunk(model_pred2, 2, dim=0)

#                         if self.args.masked_diversity:
#                             max_masks = torch.max(
#                                 batch["instance_masks"], axis=1
#                             ).values
#                             downsampled_mask = F.interpolate(
#                                 input=max_masks, size=(64, 64)
#                             )
#                             model_pred1_m = model_pred1 * downsampled_mask
#                             model_pred2_m = model_pred2 * downsampled_mask

#                             # model_pred1_cl = model_pred1.clone()
#                             # model_pred2_cl = model_pred2.clone()
#                             # model_pred1_cl = model_pred1 * downsampled_mask
#                             # model_pred2_cl = model_pred2 * downsampled_mask

#                             diversity_loss = - F.mse_loss(
#                                     model_pred1_m.float(), model_pred2_m.float(), reduction="mean"
#                             )

#                         # # diversity loss
#                         # diversity_loss = - F.mse_loss(
#                         #     (model_pred1 * downsampled_mask).float(), (model_pred2 * downsampled_mask).float(), reduction="mean"
#                         # )

#                         loss += self.args.lambda_diversity_loss * diversity_loss

#                         self.writer.add_scalar('losses/Diversity loss',diversity_loss,index)

                    # VGG loss
                    if self.args.with_vgg_loss:
                        loss_vgg = compute_vgg_loss(
                            model_pred_bck,
                            latents,
                            timesteps[-1],
                            target_bck,
                            ...
                            batch['pixel_values'][-1][None],
                            self)
                    # ...log the VGG-loss without lambda
                        self.writer.add_scalar('losses/VGG loss without lambda',loss_vgg,index)

                        loss_vgg_lam=self.args.lambda_Vgg*loss_vgg* float(not self.disable_VGG_loss)

                        loss += loss_vgg_lam
                    
                    # ...log the Total loss-with lambda
                        self.writer.add_scalar('losses/Total Loss',loss,index)
    ...
    def save_pipeline(self, path):
        ...
        if self.accelerator.is_main_process:
            ...
            if self.args.with_mask_encoder:
                me_path = os.path.join(path, "mask_encoder")
                if not os.path.exists(me_path):
                    os.makedirs(me_path)
                me_path = os.path.join(me_path, "mask_encoder.pth")
                torch.save(self.mask_encoder.state_dict(), me_path)
